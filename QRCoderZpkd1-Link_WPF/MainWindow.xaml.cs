...
namespace QRCoderZpkd1_Link
{
  public partial class MainWindow : Window
  {
    ...
    /// Преобразует PNG byte[] из Core в BitmapImage для отображения в WPF.
    private System.Windows.Media.Imaging.BitmapImage PngToImageSource(byte[] pngData)
    {
      using (var memory = new System.IO.MemoryStream(pngData))
      {
        // WPF получает уже готовый PNG и больше не использует System.Drawing для изображения.
    ...
    private void UpdatePreview()
    {
      ...
      // После изменения типа, имени или версии обновляем единую PNG-карточку.
      UpdatePreviewCard();
    }

    /// Генерирует единую PNG-карточку 490x630 для предпросмотра.
    private void UpdatePreviewCard()
    {
      string url = UrlTextBox?.Text?.Trim();

      // Без ссылки QR не генерируем — оставляем исходный логотип.
      ...
      try
      {
        string correctedUrl = LinkParser.CorrectUrl(url);
        ...
        {
          // Core создаёт готовую карточку 490x630.
          GeneratedImage card =
            PreviewCardExporter.ExportCard(
              correctedUrl,
              logoStream,
          ...
          if (QrPreviewImage != null)
          {
            // WPF только показывает готовый PNG.
            QrPreviewImage.Source = PngToImageSource(card.PngData);
          }
        }
      }
      catch
      {
        // При ошибке генерации возвращаем исходный логотип.
        if (QrPreviewImage != null)
        {
          QrPreviewImage.Source =
            new System.Windows.Media.Imaging.BitmapImage(
    ...
    private void UpdateSelectedModelsDisplay()
    {
      ...
      // Изменение списка моделей должно обновлять готовую карточку.
      UpdatePreview();
    }
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e)
    {
      ...
      if (result == true)
      {
        try
        {
          ...
          using (var logoStream = logoStreamInfo?.Stream)
          {
            // Core возвращает готовую карточку в виде PNG byte[].
            GeneratedImage finalCard =
              Core.PreviewCardExporter.ExportCard(
                correctedUrl,
                logoStream,
                typeText,
                nameText,
            ...
            // WPF только сохраняет готовые PNG-байты на диск.
